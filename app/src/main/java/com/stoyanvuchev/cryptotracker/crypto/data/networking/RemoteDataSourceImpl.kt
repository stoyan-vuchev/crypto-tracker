...
import com.stoyanvuchev.cryptotracker.crypto.data.mappers.toCoinEntity
...
class RemoteDataSourceImpl(
     ...
     * the resulting data transfer objects ([CoinsResponseDto]) to a list of [CoinEntity] objects.
     *
     * @return A [Result] containing either a list of [CoinEntity] objects on success, or a [NetworkError] on failure.
     */
    override suspend fun getCoins(): Result<List<CoinEntity>, NetworkError> {
        return safeCall<CoinsResponseDto> {
            httpClient.get(urlString = constructUrl("/ticker/24hr"))
        }.map { response ->
            response.map { it.toCoinEntity() }
        }
    }
